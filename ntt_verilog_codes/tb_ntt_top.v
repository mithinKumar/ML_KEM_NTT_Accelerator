...
module tb_ntt_top();
    ...
    task run_test_case;
        ...
        begin
            ...
            // If the memory failed to load, the first element will be 'X'
            if (expected_mem[0] === 12'bx || uut.u_coeff_mem.mem[0] === 12'bx) begin
                ...
                $finish; // Immediately stop the simulation
            end
            // -----------------------------
            ...
            for (i = 0; i < 256; i = i + 1) begin
                // 'x' check inside the loop to catch uninitialized internal logic
                if (uut.u_coeff_mem.mem[i] === 12'bx) begin
                    errors = errors + 1;
                    $display("   -> ERROR at addr %0d: RTL generated 'X' (Unknown)", i);
                end
                else if (uut.u_coeff_mem.mem[i] !== expected_mem[i]) begin
                    errors = errors + 1;
                    if (errors <= 5) begin
                        $display("   -> ERROR at addr %0d: Expected %03x, Got %03x", 
                                 i, expected_mem[i], uut.u_coeff_mem.mem[i]);
                    end
                end
            end
    ...
    initial begin
        ...
        $display("==================================================");
        $display("       STARTING NTT ACCELERATOR VERIFICATION      ");
        $display("==================================================");

        // -----------------------------------------------------------
        // TEST CASE 1: Forward NTT
        // -----------------------------------------------------------
        $readmemh("D:/download/downloads/embedded_sys_project/vivado/ntt_verilog_codes/tc1_input.hex", uut.u_coeff_mem.mem);
        $readmemh("D:/download/downloads/embedded_sys_project/vivado/ntt_verilog_codes/tc1_expected.hex", expected_mem);
        ...
        // -----------------------------------------------------------
        $readmemh("D:/download/downloads/embedded_sys_project/vivado/ntt_verilog_codes/tc2_input.hex", uut.u_coeff_mem.mem);
        $readmemh("D:/download/downloads/embedded_sys_project/vivado/ntt_verilog_codes/tc2_expected.hex", expected_mem);
        ...
        // -----------------------------------------------------------
        $readmemh("D:/download/downloads/embedded_sys_project/vivado/ntt_verilog_codes/tc3_zeros_input.hex", uut.u_coeff_mem.mem);
        $readmemh("D:/download/downloads/embedded_sys_project/vivado/ntt_verilog_codes/tc3_zeros_expected.hex", expected_mem);
        
        $display("Running Test Case 3...");
        run_test_case(3, 0);
        

        // Add as many test cases as you need here...
